...
export const glitchSpeedUpgrades = [
  ...
  },
  {
    ...
    effect: () =>  Decimal.pow(1.5, (Math.max(player.galaxies - 1e4, 0))).mul( Decimal.pow(1.2, Math.max( Replicanti.galaxies.total - 1e4, 0))).mul( Decimal.pow(1.02, Math.max( player.dilation.totalTachyonGalaxies - 1e4, 0))).max(1),
