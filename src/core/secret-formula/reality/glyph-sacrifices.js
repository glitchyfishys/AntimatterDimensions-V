...
export const glyphSacrifice = {
  ...
  "infinity": {
    ...
    effect: added => {
      ...
      const capped = Math.clampMax(sac, GlyphSacrificeHandler.maxSacrificeForEffects);
      return 1 + Math.log10(1 + Math.pow(capped, 0.2) / 100) * Math.max(amount() / 5, 1);
    },
  ...
  "time": {
    ...
    effect: added => {
      ...
      const capped = Math.clampMax(sac, GlyphSacrificeHandler.maxSacrificeForEffects);
      return Math.pow(1 + Math.pow(capped, 0.2) / 100, 2) * Math.clamp( 1.75 ** amount(), 1, 1e150);
    },
  ...
  "dilation": {
    ...
    effect: added => {
      ...
      return Math.pow(Math.clampMin(capped, 1), exponent) * Math.clamp( 1.6 ** amount(), 1, 1e150);
    },
