...
function giveRealityRewards(realityProps) {
  ...
  const realityAndPPMultiplier = multiplier + binomialDistribution(multiplier, Achievement(154).effectOrDefault(0)) + realityUGs.all[1].effectOrDefault(1);
  ...
  Currency.perkPoints.add(realityAndPPMultiplier + realityUGs.all[2].effectOrDefault(1) );
...
export function beginProcessReality(realityProps) {
  ...
  if(Perk.autoGlyph.canBeApplied){
    for (let i=0; i < player.reality.glyphs.inventory.length; i++){
      if(player.reality.glyphs.inventory[i].level < glyphLevel.actualLevel && player.reality.glyphs.inventory[i].type != "reality"){
        player.reality.glyphs.inventory[i].level = glyphLevel.actualLevel;
      }
    }
    for (let i=0; i < player.reality.glyphs.active.length; i++){
      if(player.reality.glyphs.active[i].level < glyphLevel.actualLevel && player.reality.glyphs.active[i].type != "reality"){
