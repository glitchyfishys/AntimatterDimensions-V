  ...
  methods: {
    ...
    ECc(){
      ...
      for(let i=1; i <= 12; i++){
       ...
       h == 12 ?  GameUI.notify.eternity("All EC's are completed",5000) : GameUI.notify.eternity("full completed EC's up to " + (h-1) + ", next ec" + (h) + " at " + format( new Decimal(this.ECreq[h])) + " EP",5000);
    },
  }
}
</script>
